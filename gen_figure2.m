...
num_alphas = 20;
alphas = (0:num_alphas)/num_alphas;
transforms = fraction_transform(num_alphas, @(Xs) exp(Xs));
% transforms = fraction_transform(num_alphas, @(Xs) Xs.^3);
% transforms = fraction_transform(num_alphas, @(Xs) tanh(Xs));
% transforms = fraction_transform(num_alphas, @(Xs) sigmf(Xs, [1 0]));
% transforms = fraction_transform(num_alphas, @(Xs) normcdf(Xs));
...
xlim([-0.05 1.05*max(alphas)]);
xlabel('non-Gaussian Fraction $$(\alpha)$$', 'FontSize', 12, 'Interpreter', 'latex');
...
saveas(fig, 'figs/fig2.fig');
saveas(fig, 'figs/fig2.png');
saveas(fig, 'figs/fig2.eps', 'epsc2');
