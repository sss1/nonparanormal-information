...
clear;
num_trials = 100;
ns = round(logspace(1.5, 3, 15));
p_funs = {@(n) n/4};
Sigma_funs = {@(p) corrcov(wishrnd(eye(p), p))};
transforms = {@(Xs) Xs};
...
xlabel('Sample Size $$(\log_{10}(n))$$', 'FontSize', 10, 'Interpreter', 'latex');
...
saveas(fig, 'figs/fig1.fig');
saveas(fig, 'figs/fig1.png');
saveas(fig, 'figs/fig1.eps', 'epsc2');
